...
class name_to_id_cache {
  public:
    ...
    inline void add(hash_type hash, DISPID dispid) { m_map[hash] = dispid; }
    static hash_type g_hash(const wchar_t* name) { return LHashValOfName(LANG_NEUTRAL, name); }
...
class type_info_cache {
  public:
    ...
    inline void set_type_info(ITypeInfo* type_info) { type_info_ = type_info; }

    inline bool is_valid() const { return type_info_ != nullptr; }

    inline bool is_empty() const { return !is_valid(); }

    inline ITypeInfo* get_ptr() throw() { return type_info_; }

    void init_from_typelib(ITypeLib* type_lib_ptr, const GUID& guid) { type_lib_ptr->GetTypeInfoOfGuid(guid, &type_info_); }
...
class MyIDispatchImpl : public T {
  ...
  public:
    ...
    STDMETHOD(GetTypeInfo)(unsigned int i, LCID lcid, ITypeInfo** pp) { return g_type_info_cache.GetTypeInfo(i, lcid, pp); }
...
class com_object_impl_t : public _Base {
  private:
    ...
    inline ULONG AddRef_() { return InterlockedIncrement(&m_dwRef); }

    inline ULONG Release_() { return InterlockedDecrement(&m_dwRef); }
